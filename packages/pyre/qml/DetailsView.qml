...
// Details (columnar) view — sortable columns (PRD §5.3).
Item {
    id: root
        ...
        delegate: Rectangle {
            ...
            MouseArea {
                ...
                onPositionChanged: (mouse) => {
                    if (!_drag || ma.Drag.active) return
                    if (Math.abs(mouse.x - _px) > _dragThreshold || Math.abs(mouse.y - _py) > _dragThreshold) {
                        ma.Drag.active = true
                        ma.Drag.source = ma
                        ma.Drag.mimeData = { "text/uri-list": model.filePath }
                        ma.Drag.keys = ["text/uri-list"]
                    }
                }
                onClicked: (mouse) => {
    ...
    Keys.onPressed: (event) => {
        ...
        if (text.length === 1 && text >= " " && text !== " " &&
            ...
            detBuf = (detBuf + text).slice(-64)
            fsModel.typeahead(detBuf)
            event.accepted = true
        }
    }
    property string detBuf: ""
    focus: true
}
